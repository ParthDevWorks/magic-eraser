...
def model_initialized():
    return get_inpainitng_model(id="runwayml", initialize=True)


@pytest.fixture(scope="module")
def model_uninitialized():
    return get_inpainitng_model(id="runwayml", initialize=False)
...
def test_correct_factory_initialized(model_initialized):
    assert model_initialized.get_model_id() == "runwayml"


def test_correct_factory_uninitialized(model_uninitialized):
    assert model_uninitialized.get_model_id() == "runwayml"
...
def test_check_initialization(model_initialized):
    assert isinstance(model_initialized, RunWayML)
...
def test_shutdown(image, mask):
    model = get_inpainitng_model(id="runwayml", initialize=True)
    model.shutdown()
